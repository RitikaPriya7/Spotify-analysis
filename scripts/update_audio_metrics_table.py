...
DEFAULT_METRICS_PATH = Path("data/results/audio_classifier_metrics.csv")
...
def build_table_block(rows: str) -> str:
    return (
        ...
        "    \\label{tab:audio-metric-results}\n"
        "  \\end{table}\n"
        "}\n"
    )
...
def replace_table_block(tex_path: Path, new_block: str) -> None:
    ...
    pattern = re.compile(
        r"(\\newcommand{\s*\\audioMetricResultTable}{%\s*\n)(.*?)(\n}\s*)",
        re.DOTALL,
    )
    match = pattern.search(content)
    if not match:
        raise ValueError(
            f"Could not locate audioMetricResultTable definition in {tex_path}"
        )
