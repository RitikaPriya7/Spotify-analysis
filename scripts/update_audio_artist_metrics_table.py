...
DEFAULT_METRICS_PATH = Path("data/results/audio_artist_classifier_metrics.csv")
...
def build_table_rows(
    ...
    for split_idx, split in enumerate(SPLITS):
        ...
        for metric_key, metric_label in METRIC_ORDER:
            row_values: List[str] = []
            for _, model_key in MODEL_DISPLAY:
...
def build_table_block(rows: str) -> str:
    return (
        ...
        "      \\bottomrule\n"
        "    \\end{tabular}\n"
        "    \\caption{Comparison of model performance across all evaluation metrics using audio + artist dataset split.}\n"
        "    \\label{tab:audio-and-artist-metric-results}\n"
        "  \\end{table}\n"
        "}\n"
    )
...
def replace_table_block(tex_path: Path, new_block: str) -> None:
    ...
    pattern = re.compile(
        r"(\\newcommand{\s*\\audioAndArtistMetricResultTable}{%\s*\n)(.*?)(\n}\s*)",
        re.DOTALL,
    )
    match = pattern.search(content)
    if not match:
        raise ValueError(
            f"Could not locate audioAndArtistMetricResultTable definition in {tex_path}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Update the LaTeX table with Train/Test metrics for the audio + artist dataset."
        )
    )
    parser.add_argument(
        "--metrics-path",
        ...
        help="Path to the audio+artist metrics CSV.",
    )
    parser.add_argument(
        "--tex-path",
        type=Path,
